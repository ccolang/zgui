...
void zgui::begin_groupbox(std::string_view title, const vec2 size, const int flags) {
  ...
  const bool centered = flags & zgui_groupbox_flags_title_centered;
  if (flags & zgui_groupbox_flags_legacy_design) {
    ...
    if (!title.empty()) {
      int x = centered ? draw_pos.x + (size.x / 2) : draw_pos.x + 4;

      functions.draw_text(x, draw_pos.y - 8, global_colors.color_text, font, centered, title.data());
    }
  }
  else {
    functions.draw_rect(draw_pos.x - 1, draw_pos.y - 1, size.x + 2, size.y + 2, global_colors.control_outline);
    ...
    if (!title.empty()) {
      int x = centered ? draw_pos.x + (size.x / 2) : draw_pos.x + 4;
      functions.draw_text(x, draw_pos.y + 2, global_colors.color_text, font, centered, title.data());
    }
  }
